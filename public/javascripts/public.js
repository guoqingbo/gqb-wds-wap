...
$(function () {
	...
	if ($('.index_news_scroll').length > 0) {
		$('.index_news_scroll').slide({
			mainCell: '.bd',
			effect: 'topLoop',
			autoPlay: true,
			interTime: 5000
 ...
 * 将form转为AJAX提交
 */
function formToAjaxSubmit(frm, returnUrl) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			if ('200' == data.status || data.success) {
				if ('true' == data.showWindow)
					//dilogbox(data.status,data.message,returnUrl);
					alert(data.message);
				else
		...
		error: function (data) {
			...
			//dilogbox(data.status,data.responseText,returnUrl);
		}
	});
}
...
function addLabels() {
	$('.labels').each(function () {
		var $t = $(this);
		var txt_arr = $t.text().split(',');
		var dom = '';
		$.each(txt_arr, function (i, t) {
			if (i > 1) {
				dom += '<span class="pro-flag c-base border-base">...</span>';
				return false
			}
			dom += '<span class="pro-flag c-base border-base">' + t + '</span>';
		});
		$t.after(dom).remove();
	});
}