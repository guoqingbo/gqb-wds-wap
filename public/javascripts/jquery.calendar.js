;(function ($){
    ...
    var calendar = {
        ...
        // 构造确认按钮
        getConfirmBtn: function (opts){
            return $('<div class="calendar_confirm">确认</div>').on('click',function (){
                opts.confirmClick(private.selectedDate);
            });
        },
        // 构建月份
        ...
        console.log(nDay)
                ...
                if (opts.settingdata.length){
                    ...
                    var filterArr = opts.settingdata
                        ...
                        _pd = filterArr.length ? '<span>' + (filterArr[0].price ? '￥' + filterArr[0].price : '无价格') + '</span><strong>' + (filterArr[0].stock || '无库存') + '</strong>' : '';
            ...
            return $('<ul class="calendar_body">' + monthDom.join('')+ '</ul>').on('click','li:not(.item-empty)',function (){
                ...
                private.selectedDate = reArr.length ? reArr : selectedArr;
        ...
        init: function (obj,opts){
            ...
            obj.append(_this.getConfirmBtn(opts));
        }
    };
